...
function innerRpc(request, callback) {
    ...
    var errorMessage = 'xazab JSON-RPC: ';
    ...
    var req = this.protocol.request(options, function (res) {
        ...
        res.on('end', function () {
            ...
            if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
                var exceededError = new Error('xazab JSON-RPC: ' + buf.toString('utf8'));
...
// For definitions of RPC calls, see various files in: https://github.com/xazabpay/xazab/tree/master/src
RpcClient.callspec = {
